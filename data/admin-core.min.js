...
function wp_lib_render_notification(notificationText){var formattedNotification=wp_lib_format_notification(notificationText);if(notificationText[0]!=0&&typeof console!=='undefined'){console.log(formattedNotification.consoleMessage);}
notificationHolder.append(formattedNotification.element).hide().fadeIn(500);var displayTime=notificationText[1].length*150;if(displayTime<3500){displayTime=3500;}else if(displayTime>7000){displayTime=9000;}
setTimeout(function(){wp_lib_hide_notification(formattedNotification.element);},displayTime);}
...
function wp_lib_format_notification(notification){var classes,message,consoleMessage,uID=Math.random().toString().substring(2,8);if(notification[0]==0){classes='wp-lib-notification updated';message=notification[1];}else if(notification[0]==1){classes='wp-lib-error error';consoleMessage='WP-Librarian Error: '+notification[1];message='<strong style="color: red;">'+consoleMessage+'</strong>';}else{classes='wp-lib-error error';consoleMessage='WP-Librarian Error '+notification[0]+': '+notification[1];message='<strong style="color: red;">'+consoleMessage+'</strong>';}
return{'element':jQuery('<div id="'+uID+'" onclick="wp_lib_hide_notification(this)" class="'+classes+'"><p>'+message+'</p></div>'),'consoleMessage':consoleMessage};}
function wp_lib_hide_notification(notification){notification.fadeOut("fast");notification.remove();}
